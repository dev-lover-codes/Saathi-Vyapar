...
export default async function FacilitatorPage() {
  ...
  return (
    <div className="min-h-screen bg-black text-white p-3 sm:p-6 pb-24 font-sans">
      {/* ── Top Header ────────────────────────────────────────────── */}
      <header className="max-w-6xl mx-auto flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-800 pb-4 mb-6">
        <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            href="/dashboard"
            className="px-3 py-2 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 text-xs font-bold rounded-xl border border-zinc-700"
          >
      ...
      <main className="max-w-6xl mx-auto space-y-6">
        ...
        <section className="grid grid-cols-1 sm:grid-cols-4 gap-4">
          <div className="bg-zinc-900 border-2 border-zinc-700 rounded-2xl p-4 shadow-lg">
            <span className="text-zinc-400 text-xs font-bold uppercase">कुल पंजीकृत उद्यमी</span>
            <p className="text-3xl font-black text-white mt-1">{entrepreneurs.length}</p>
            <p className="text-[11px] text-zinc-500 mt-1">Total Registered Entrepreneurs</p>
          </div>

          <div className="bg-zinc-900 border-2 border-zinc-700 rounded-2xl p-4 shadow-lg">
            <span className="text-zinc-400 text-xs font-bold uppercase">प्लान जनरेटेड</span>
            <p className="text-3xl font-black text-emerald-400 mt-1">
              {entrepreneurs.filter((e) => e.lastPlanDate).length}
            </p>
            <p className="text-[11px] text-zinc-500 mt-1">Active Financial Plans</p>
          </div>

          <div className="bg-zinc-900 border-2 border-zinc-700 rounded-2xl p-4 shadow-lg">
            <span className="text-zinc-400 text-xs font-bold uppercase">औसत मासिक आय</span>
            ...
            <p className="text-[11px] text-zinc-500 mt-1">Avg. Monthly Revenue</p>
          </div>

          <div className="bg-zinc-900 border-2 border-zinc-700 rounded-2xl p-4 shadow-lg">
            <span className="text-zinc-400 text-xs font-bold uppercase">उच्च जोखिम मामले</span>
            <p className="text-3xl font-black text-rose-400 mt-1">
              {entrepreneurs.filter((e) => e.cashFlowRisk === 'high').length}
            </p>
            <p className="text-[11px] text-zinc-500 mt-1">High Risk Profiles</p>
          </div>
        </section>

        {/* ── Entrepreneurs Table ───────────────────────────────────── */}
        <section className="bg-zinc-900 border-2 border-zinc-700 rounded-2xl p-5 shadow-2xl space-y-4">
          ...
          {entrepreneurs.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="border-b border-zinc-800 text-zinc-400 text-xs uppercase font-extrabold">
                ...
                <tbody className="divide-y divide-zinc-800">
                  {entrepreneurs.map((item) => (
                    <tr key={item.id} className="hover:bg-zinc-800/50 transition-colors">
                      ...
                      <td className="py-3 px-3">
                        <span className="px-2.5 py-1 bg-zinc-800 border border-zinc-700 rounded-md text-xs text-zinc-300 font-medium">
                          {item.sector}
                        </span>
                      </td>
                      <td className="py-3 px-3">
                      ...
                      <td className="py-3 px-3">
                        {item.marginPercent !== null ? (
                          ...
                          <span className="text-xs text-zinc-500">कोई प्लान नहीं</span>
                        )}
                      </td>
                      ...
                      <td className="py-3 px-3 text-right">
                        <Link
                          ...
                          className="inline-flex items-center px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-lg transition-colors"
                        >
